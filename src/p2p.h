...
struct PeerState {
    ...
    // in-flight block request (for stall detection)
    int64_t     inflight_since_ms{0};
};
...
private:
    ...
    // known IPv4 addrs (network byte order)
    std::set<uint32_t> addrv4_;

    std::string datadir_{"./miqdata"};

    // outbound management
    static constexpr int kTargetOutbound = 8;
    int64_t last_outbound_dial_ms_{0};
    ...
    // outbound dialing
    void maybe_outbound_dial();

    // version payload helper (back-compat with empty)
    void send_version(int s);
};
