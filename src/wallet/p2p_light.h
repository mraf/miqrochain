...
struct P2POpts {
    ...
    std::string port;            // e.g. "9333" (falls back to P2P_PORT if empty)
    std::string user_agent;      // e.g. "/miqwallet:0.1/"
    uint32_t    start_height{0};
    int         io_timeout_ms{10000};
    bool        send_verack{true}; // keep true; node is strict about verack timing
};
...
private:
    ...
    bool send_verack(std::string& err);  // <-- added declaration
