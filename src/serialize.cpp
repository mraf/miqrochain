...
static inline bool read_hash_field(const std::vector<uint8_t>& b, size_t& i, std::vector<uint8_t>& out){
    ...
    if (i + 32 <= b.size()) {
        out.assign(b.begin() + i, b.begin() + i + 32);
        i += 32;
        return true;
    }

    if (i + 4 > b.size()) return false;
    uint32_t len = get_u32(b, i);
    i += 4;
...
bool deser_block(const std::vector<uint8_t>& b, Block& out){
    ...
    out.header.version = get_u32(b, i);
    i += 4;
