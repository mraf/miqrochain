...
static bool parse_object(const std::string& s, size_t& i, JNode& out) {
    ...
    while (true) {
        std::string k;
        if (!parse_string(s, i, k)) return false;
        skip(s, i);
        if (i >= s.size() || s[i] != ':') return false;
        ++i;
        skip(s, i);
...
static bool parse_number(const std::string& s, size_t& i, double& out) {
    ...
    // Keep original behavior: rely on stod throwing on bad input.
    out = std::stod(s.substr(j, i - j));
    return true;
}
...
static bool parse_value(const std::string& s, size_t& i, JNode& out) {
    ...
    double num;
    if (parse_number(s, i, num)) { out.v = num; return true; }
    return false;
}

bool json_parse(const std::string& s, JNode& out) {
    size_t i = 0;
    bool ok = parse_value(s, i, out);
    if (!ok) return false;
    skip(s, i);
    return i == s.size();
}
...
static void dump(const JNode& n, std::ostringstream& o) {
    if (std::holds_alternative<JNull>(n.v)) o << "null";
        ...
        o << '[';
        const auto& a = std::get<std::vector<JNode>>(n.v);
        for (size_t i = 0; i < a.size(); ++i) { if (i) o << ','; dump(a[i], o); }
        o << ']';
    } else {
        o << '{';
        const auto& m = std::get<std::map<std::string, JNode>>(n.v);
        size_t i = 0;
        for (auto& kv : m) { if (i++) o << ','; o << '"' << kv.first << '"' << ':'; dump(kv.second, o); }
        o << '}';
    }
}

std::string json_dump(const JNode& n) {
    std::ostringstream o; dump(n, o); return o.str();
}
