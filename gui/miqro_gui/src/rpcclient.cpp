...
if (m_settings.autoReadCookie()) {
return util::readCookieToken(err);
}
return m_settings.manualToken();
}


QJsonValue RpcClient::call(const QString &method, const QJsonArray &params){
...
QEventLoop loop; QTimer timer; timer.setSingleShot(true); timer.setInterval(30000);
...
QJsonValue RpcClient::sendFromHd(const QString &to, const QString &amt, quint64 fee){ return call("sendfromhd", QJsonArray{ to, amt, QString::number(fee) }); }
